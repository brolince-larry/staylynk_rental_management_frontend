...
import { useAdminMessages, useAdminMessageThread, useAdminMessageRecipients, useSendAdminMessage } from '../hooks/index'
...
export default function AdminMessages(): React.ReactElement {
  ...
  const { data, isLoading } = useAdminMessages({ per_page: 30 })
  const { mutate: send, isPending: sending } = useSendAdminMessage()
  const { data: recipients = [], isLoading: recipientsLoading } = useAdminMessageRecipients()
  const { data: thread, isLoading: threadLoading } = useAdminMessageThread(selectedId)
